...
class Analysis{
    ...
    static function analyze_power_word($analyzes, $data){
        ...
        if(!empty($data['power_words'])){
            $analyzes['power_words'] = [
                'desc' => '<p>Good: You have power words in your title.</p>',
                'impact' => 'good',
				'title'=>'Power Word in title'
            ];
        } else{
            $analyzes['power_words'] = [
                'desc' => '<p>Consider adding power words to your title for better impact.</p>',
                'impact' => 'low',
				'title'=>'Power Word in title'
            ];
        }
    ...
    static function analyze_title_number($analyzes, $data){
        ...
        if(!empty($data['number_found'])){
            $analyzes['number_found'] = [
                'desc' => sprintf('<p>Good: Your title contains the number "%s".</p>', $data['number_found']),
                'impact' => 'good',
				'title' => 'Number in title'
            ];
        } else{
            $analyzes['number_found'] = [
                'desc' => '<p>Consider adding a number to your title to improve CTR.</p>',
                'impact' => 'low',
				'title' => 'Number in title'
            ];
        }
    ...
    static function analyze_passive_voice($analyzes, $data){
        ...
        if(empty($data['passive_voice'])){
            $analyzes['passive_voice'] = [
                'desc' => '<p>Great: No passive voice detected.</p>',
                'impact' => 'good',
				'title' => 'Passive Voice'
            ];
        } else {
            ...
            if($percentage <= 10){
                $analyzes['passive_voice'] = [
                    'desc' => sprintf('<p>Good: Only %s%% of sentences use passive voice.</p>', $percentage),
                    'impact' => 'good',
					'title' => 'Passive Voice'
                ];
            } elseif($percentage < 20){
                $analyzes['passive_voice'] = [
                    'desc' => sprintf('<p>Okay: %s%% of sentences use passive voice. Try to reduce it.</p>', $percentage),
                    'impact' => 'medium',
					'title' => 'Passive Voice'
                ];
            } else{
                $analyzes['passive_voice'] = [
                    'desc' => sprintf('<p>High: %s%% of sentences use passive voice. Consider revising.</p>', $percentage),
                    'impact' => 'high',
					'title' => 'Passive Voice'
                ];
            }
    ...
    static function analyze_paragraph_length($analyzes, $data){
        ...
        if(empty($data['paragraph_length']) || $data['paragraph_length'] < 150){
            ...
            $analyzes['paragraph_length'] = [
                'desc' => sprintf('<p>Consider reducing paragraph length. Current length: %s words.</p>', $data['paragraph_length']),
                'impact' => 'low',
				'title' => 'Paragraph Length'
            ];
        }
					...
					if(!empty($seo_analysis)){
						...
						$html .= '<div class="siteseo-analysis-summary-pill">';
							...
							if(!empty($seo_analysis['error_count'])){
								$html .= '<span><svg fill="#f33" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M24 22h-24l12-20z"/></svg>'.esc_html($seo_analysis['error_count']). ' Errors</span>';
							}
							...
							if(isset($check['label'])){
								$html .= '<div class="siteseo-analysis-block-title">';
									if(isset($check['status_class'])){
										...
										$html .= '<div><span class="impact '.esc_attr($check['status_class']).'" aria-hidden="true">'.$impact_icon.'</span>
										<span class="screen-reader-text">'. sprintf(esc_html__('Degree of severity: %s','siteseo'), esc_html($check['status_class'])).'</span>';
									}

									$html .= esc_html($check['label']).'</div>
									<span class="siteseo-arrow" aria-hidden="true"></span>
								</div>';
							}
							if(isset($check['details'])){
								$html .= '<div class="siteseo-analysis-block-content" aria-hidden="true">'.wp_kses_post($check['details']).'</div>';
							}
	...
	static function check_meta_description($content, $meta_description = '', $keywords = []){
		...
		$description = !empty($meta_description) ? $meta_description : wp_trim_words($content, 20);
			...
			foreach($internal_links as $link){
				...
				if($post_id){
					$details .= '<a class="nounderline" href="' . get_edit_post_link($post_id) . '" ' .
							   'title="' . sprintf(__('edit %s', 'siteseo'), esc_html(get_the_title($post_id))) . '">' .
	...
	static function analyze_nofollow_links($content){
		...
		if($total_nofollow > 0){
			$details .= '<p>' . sprintf(
				esc_html__('We found %d links with the nofollow attribute on your page. Avoid overusing the nofollow attribute in links. Below is the list:', 'siteseo'),
				$total_nofollow
			) . '</p>';
