...
class Tools{
		...
		echo '<form method="post" id="siteseo-form" class="siteseo-option" name="siteseo-flush">
				...
				<div style="siteseo-tools-page">
					...
					<div class="siteseo_wrap_label"><div class="siteseo-notice is-warning">
						...
						<div><p>'.wp_kses_post('<strong>WARNING:</strong> Delete all options related to this plugin in your database.','siteseo').'</p></div>
					</div></div>
	...
	static function display_plugins($plugin,$name){
		...
		<div class="inside">
		<h3>'.sprintf(esc_html__('Import posts and terms (if available) metadata from %s', 'siteseo'), esc_html($name)).'</h3>
					...
					<div class="siteseo_wrap_label">						
						<div class="siteseo-notice is-warning">
							<span id="dashicons-warning" class="dashicons dashicons-warning"></span>&nbsp;
								<p>'.sprintf(__('<strong>WARNING:</strong> Migration will delete / update all <strong>%1$s posts and terms metadata</strong>. Some dynamic variables will not be interpreted. We do <strong>NOT delete any %2$s data</strong>.', 'siteseo'), esc_html($seo_title), esc_html($name)) . '
								</p>
						</div>
					</div>
